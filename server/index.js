 ...
 * Port : Railway utilise process.env.PORT
...
app.post('/api/decisions-cal', requireAuth, requireRole('agent', 'directeur'), (req, res) => {
  ...
  log(
    req.user,
    'DECISION_CAL',
    `Logement ${logement_ref} · ${candidats?.length} candidats · rang 1 : ${
...
app.post('/api/match-candidat', (req, res) => {
  ...
  if (!match && nom && prenom) {
    const candidates = demandeurs.filter(
      (d) =>
        normalize(d.nom) === normalize(nom) &&
        normalize(d.prenom) === normalize(prenom)
    );
...
app.get('/api/dashboard', (req, res) => {
  ...
  const delaiMoyen = attribues.length
    ? Math.round(
        attribues.reduce(
          (s, a) =>
            s + a.jours_audience_proposition + (a.jours_proposition_attribution || 0),
          0
        ) / attribues.length
      )
    : null;
